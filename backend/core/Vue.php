...
class Vue
{
    ...
    {
        $basePath = rtrim(str_replace('\\', '/', $this->cheminVues), '/');

        // Autoriser uniquement: lettres, chiffres, point, slash
        if (!preg_match('/^[a-zA-Z0-9._\/]+$/', $vue)) {
            throw new \Exception("Nom de vue invalide: $vue");
        }

        $vuePath = str_replace('.', '/', $vue);

        $chemin = $basePath . '/' . $vuePath . '.php';

        $realBase = realpath($basePath);
        $realFile = realpath($chemin);

        // empêche sortie du dossier views
        if ($realFile && strpos($realFile, $realBase) !== 0) {
            throw new \Exception("Accès vue interdit");
        }
