...
class Requete
{
    ...
    /**
     *recupere tous les en-têtes HTTP
     */
    public function tousEntetes(): array
    {
        $entetes = [];
        foreach ($this->server as $cle => $valeur) {
            if (strpos($cle, 'HTTP_') === 0) {
                // Convertir HTTP_X_HEADER_NAME en X-Header-Name
                $nomEntete = str_replace('_', '-', substr($cle, 5));
                $nomEntete = strtolower($nomEntete);
    ...
    /**
     * Décode et retourne les données JSON du corps de la requête
     */
    ...
    /**
     * Obtient un header spécifique
     */
    public function header(string $nom): ?string
    {
        $headers = $this->headers();
        $nom = strtolower($nom);
        return $headers[$nom] ?? null;
    }
}
