  ...
  pageData.forEach((d) => {
    ...
    tr.innerHTML = `
                <td class="px-6 py-4 text-sm font-medium">
                  ${new Date(d.date_versement).toLocaleDateString("fr-FR", { day: "2-digit", month: "short", year: "numeric" })}
                </td>
                ...
                <td class="px-6 py-4 text-lg font-extrabold text-right">
                  ${Number(d.montant).toFixed(2)} $
                </td>
                <td class="px-6 py-4 text-center">
                  <span class="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-bold uppercase ${statusClass}">
                    ${statusText}
                  </span>
                </td>
                ...
                <td class="px-6 py-4">
                  <div class="flex items-center justify-center gap-2">
                    ...
                    <!-- Edit -->
                    <button 
                      onclick="editVersement(${d})"
                      class="group p-2 rounded-lg bg-blue-500/10 hover:bg-blue-500 text-blue-500 hover:text-white transition-all"
                      title="Modifier"
                    >
                    ...
                    <!-- Delete -->
                    <button 
                      onclick="deleteVersement(${d.id})"
                      class="group p-2 rounded-lg bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white transition-all"
                      title="Supprimer"
                    >
...
function editVersement(d) {
  document.getElementById("date").value = d.date_versement;
  document.getElementById("montant").value = d.montant;
  document.getElementById("depense").value = d.depense;
  document.getElementById("entrepot").value = d.entrepot_id;
